...
export async function recommend(opts: { stepId: number; userId: number }) {
  ...
  async function runProposalRound(broader: boolean): Promise<VerifiedRec[]> {
    ...
    for (const c of candidates) {
      ...
      // Post-resolve rated check: MusicBrainz canonicalizes title/artist, so a
      // rated track the LLM spelled differently is caught here.
      ...
      // Drop intra-round duplicates (the LLM can repeat a track, or two
      // candidates can resolve to the same recording).
